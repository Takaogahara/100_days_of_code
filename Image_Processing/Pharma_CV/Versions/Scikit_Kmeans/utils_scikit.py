...
def k_mean(imagem):
    ...
    for kmeans_iter in range(3, 10):
        kmeans_clf_wcss = KMeans(n_clusters = kmeans_iter, init = 'k-means++', n_init=3, max_iter=5)
        kmeans_clf_wcss.fit((image_list))
        wcss.append(kmeans_clf_wcss.inertia_)
    K = optimal_number_of_clusters(wcss)

    kmeans_clf = KMeans(n_clusters = K, init = 'k-means++', n_init=3, max_iter=5)
    kmeans_clf.fit((image_list))
    kmeans_centroids = kmeans_clf.cluster_centers_
    kmeans_labels = kmeans_clf.labels_

    image_kmeans = kmeans_centroids[kmeans_labels]
    image_kmeans = image_kmeans.reshape((imagem.shape[0], imagem.shape[1], 3))
